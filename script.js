...
// EVENTS
// --- REPLACE THE BOTTOM PART OF SCRIPT.JS WITH THIS ---

document.addEventListener('DOMContentLoaded', () => {
    console.log("System Initialized.");
    
    // 1. URL CHECKER (Ye naya logic hai)
    // Check karega ki kya Home page ne koi order (query) bheja hai?
    const urlParams = new URLSearchParams(window.location.search);
    const queryFromHome = urlParams.get('q');

    if (queryFromHome) {
        // Agar Home se aye ho, toh wo search karo
        handleSearch(queryFromHome);
    } else {
        // Agar direct aye ho, toh default search
